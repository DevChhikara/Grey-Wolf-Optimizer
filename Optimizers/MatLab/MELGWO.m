...
% Article Name: Memory, Evolutionary Operator, and Local Search Based Improved Grey Wolf Optimizer 
% with Linear Population Size Reduction Technique
% DOI: https://doi.org/10.1016/j.knosys.2023.110297 
% The algorithm was developed by Rasel Ahmed, Emeritus Professor Gade Pandu Rangaiah,
% and Professor Seyedali Mirjalili with significant help from the other co-authors(in the year 2022)
% The intention of making this code open source is to enrich the knowledge domain 
% and codes related to GWO, and also to help the research community.
% If you use this code, and/or get benifited from our work, please cite our work. 
% We would be happy if you found our code and article useful to you and your project. 

% The basic GWO algorithm part of this MELGWO is taken and slightly modified from the original code
% developed by professor Seyedali Mirjalili, we are thankful to professor Seyedali Mirjalili and 
% his team for this. The DOI of the article: http://dx.doi.org/10.1016/j.advengsoft.2013.12.007
% The local search technique used in this algorithm is adapted, and further modified from the Niching 
% PSO algorithm developed by Professor P-N-Suganthan and his team. 
% The DOI of the article: http://dx.doi.org/10.1016/j.ins.2012.02.011

% This code is mainly designed for solving basic optimization (mininmization) problems with real variables, 
% but it can also work for high dimensional optimization problems, and problems with inequality constraints. 
% To solve problems with inequality, you need to adapt an ineqaulity constraint handling 
% technique with the algorithm. So far, we didn't test the algortihm for
% equality constrant problems. But, we would like to encourage researcher to adapt any efficient equality-inequality 
% constraint handling technique to this algorithm and apply it for various types of optimization problems. 
% Finally, thank you so much for reading our article, and looking into our code. 
...
nVar = 30;            % Maximum number of Decision Variables or Dimensions of the problem 
m = [1 nVar];         % Size of Decision Variables Matrix
lb = -100;            % Lower bound
ub = 100;             % Upper bound 
Agents_no = 100;      % Number of search agents
...
% If each variable has a different lb and ub (such as lb = [-10, -100, -2, -4.5]; ub = 10, 100, 2, 4.5]);
% if Boundary_no > 1
%     for i=1:dim
%         ub_i=ub(i);
%         lb_i=lb(i);
%         Agents(i).Position = unifrnd(lb,ub,dimsize);
%     end
% end
